...
Public Class Host_IndentitasCadebBM
        ...
        If Not IsPostBack Then
            ...
            With oDs.Tables(0)
                ...
                If (.Rows(x).Item("statusid") = "DROPD") Then
                    ...
                    'filldropdownlistcancel()
                    reactaction.Visible = False
                    reactdisplay.Visible = False
                    txtnotescra.ReadOnly = True
                Else
                    lblactivity.Text = "View Data"
                    'btnJJSVY.Visible = False
                    btnBMEND.Visible = False
                    btnBMREQ.Visible = False
                    txtnotes.ReadOnly = True
                    txtnotescra.ReadOnly = True
                    If (.Rows(x).Item("statusid") = "DROPD") Then
                        reactaction.Visible = False
                        reactdisplay.Visible = False
                        notereact.Visible = False
                    Else
                        reactaction.Visible = False
                        reactdisplay.Visible = False
                        notereact.Visible = False
                ...
                txtnotescra.Text = .Rows(x).Item("canote")
                txtnotes.Text = .Rows(x).Item("bmnote")
    ...
    Sub DropDownList1_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs)
        cancelid.Value = DropDownList1.SelectedItem.Value
    End Sub
    Private Sub filldropdownlistcancel()
        Dim oDs As DataSet
        Dim oLoad As New ClsApp
        Dim OComm As New clsCommon
        Dim nCol As Byte
        ...
        tblStatus = New DataTable("liststatus")
        reffnumber = Request.QueryString("reID")
        oDs = oLoad.displaydropdownlistcancel(reffnumber)
        nCol = oDs.Tables(0).Columns.Count - 1
        nRow = oDs.Tables(0).Rows.Count - 1
        DropDownList1.DataSource = oDs.Tables(0)
        DropDownList1.DataTextField = "canceldesc"
        DropDownList1.DataValueField = "cancelid"
