...
Public Class Host_IndentitasCadebCA
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Session("reffnumber") = reffnumber
        ''#################################
        btncekasiri.Visible = False
        TabSelector(1)
        If Not Me.IsPostBack Then
            Session("reffnumber") = reffnumber
            Dim oQry As New clsQry
            Dim oQry1 As New clsQry
            Dim sMsg As String = ""
            Dim nRowList2 As Integer
            Dim oDsur As DataSet
            Dim usrRequestSurvey As String
            Dim y As Integer
            ...
            With oDsmarq.Tables(0)
                ...
                phonex_result = phonex_result.ToUpper()
                phonex_total = phonex_total.ToUpper()
            ...
            'bagian untuk block by ca 
            oDsur = oQry1.getDataRequestSurvey(reffnumber) 'untuk cek jika ada di table dfc_blockByCA
            With oDsur.Tables(0)
                nRowList2 = oDsur.Tables(0).Rows.Count
                nRowList2 = oDsur.Tables(0).Rows.Count - 1
                If oDsur.Tables(0).Rows.Count <> 0 Then
                    'MsgBox("ada")
                    'islocked = True

                    usrRequestSurvey = .Rows(y).Item("user_CA")
                    If usrRequestSurvey = userlogin Then
                        islocked = False
                    Else
                        islocked = True
                    End If
                Else
                    'MsgBox("tidak ada")
                    islocked = False
                    sMsg = oQry.inserttempRequestSurvey(reffnumber, userlogin)
                End If
            End With
            ...
            With oDs.Tables(0)
                ...
                nRowList = oDs.Tables(0).Rows.Count
                nRowList = oDs.Tables(0).Rows.Count - 1
                If nRowList >= 0 Then
                    If ((.Rows(x).Item("statusid") = "CBASC") Or (.Rows(x).Item("statusid") = "BMREQ")) Then
                        lblactivity.Text = "Set Status"
                        If .Rows(x).Item("statusid") = "CBASC" Then
                            divnotebm.Visible = False
                        Else
                            divnotebm.Visible = True
                            txtnotesbm.ReadOnly = True
                        End If
                        If islocked = True Then
                            btnRQSVY.Visible = False
                            btnDROPD.Visible = False
                        End If
                    Else
                        ...
                        If (.Rows(x).Item("statusid") = "DROPD") Then
                            divnotebm.Visible = False
                        Else
                            divnotebm.Visible = True
                            txtnotesbm.ReadOnly = True
                    ...
                    txtsumberdata.Text = .Rows(x).Item("sumberdata")   'tutup sementara
                    ...
                    txtcekreffnumber.Value = reffnumber
                    txtceknationalid.Value = .Rows(x).Item("nationalId")
                    txtcekname.Value = .Rows(x).Item("name")
                    txtcekbirthdate.Value = .Rows(x).Item("birthDate")
                    txtcektempatlahirdebitur.Value = tempatlahirdebitur
                    txtcekmobile.Value = .Rows(x).Item("mobile")
                    txtcekvaruserid.Value = var_userid
                    ...
                    If lblactivity.Text = "View Data" Then
            ...
            'sTable = ""
            'sTable = "<table cellspacing='0' cellpadding='0' border='0'>"
    ...
    End Class
    Protected Sub btncekasiri_Click(sender As Object, e As EventArgs) Handles btncekasiri.Click
    ...
    Private Sub getasliri()
        cekreffnumber = txtcekreffnumber.Value
        ceknationalid = LTrim(RTrim(txtceknationalid.Value))
        cekname = LTrim(RTrim(txtcekname.Value))
        cekbirthdate = LTrim(RTrim(txtcekbirthdate.Value))
        cektempatlahirdebitur = LTrim(RTrim(txtcektempatlahirdebitur.Value))
        cekmobile = LTrim(RTrim(txtcekmobile.Value))
        cekvaruserid = txtcekvaruserid.Value
        'cekasliri("34332", "1671070509830013",  "RICKY ALFIAN", "09/05/1983", "JAKARTA BARAT", "082280112533", "Indri_CA")
        cekasliri(cekreffnumber, ceknationalid, cekname, cekbirthdate, cektempatlahirdebitur, cekmobile, cekvaruserid)
        Exit Sub
    End Sub
    Public Sub cekasliri(ByVal ReqID As String, ByVal NIK As String, ByVal Name As String, ByVal dob As String, ByVal pob As String, ByVal handphone As String, ByVal sUserID As String)
        ...
        Try
            ...
            phonex_result = json.SelectToken("data.Phone_extra_result")
            phonex_total = json.SelectToken("data.Phone_extra_total")
            ...
            Dim oAdapter As New SqlDataAdapter(sQuery, sqlconapps)
            Dim oDs2 As New DataSet
            oAdapter.Fill(oDs2, "DataSet")    'mengisi NOT FOUND ke tabel datafromchannel _x di kolom negative_record
